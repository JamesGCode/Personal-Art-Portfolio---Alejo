...
import { fetchImages, fallbackImages } from "./data/images.js";
...
async function init() {
  ...
  try {
    ...
    console.warn("No se pudo cargar la API, usando imágenes locales.", err);
  ...
  // ── Carga la primera imagen de la API al arrancar ──
  const firstEntry = images[0];
  imgTop.src = firstEntry.main;
  imgBehind.src = firstEntry.behind;
  ...
  function animateDigit(container, newValue, direction) {
    ...
    container
      .querySelectorAll("span:not(.active)")
  ...
  container.addEventListener(
    ...
    (e) => {
      ...
      targetTranslate = Math.min(
        Math.max(targetTranslate - v, -maxTranslate),
        0,
      );
    },
    { passive: false },
